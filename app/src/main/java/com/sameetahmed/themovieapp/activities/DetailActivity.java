...
import com.sameetahmed.themovieapp.adapter.TrailerAdapter;
import com.sameetahmed.themovieapp.data.MovieContract;
import com.sameetahmed.themovieapp.model.Movie;
import com.sameetahmed.themovieapp.model.Trailer;
...
import static com.sameetahmed.themovieapp.data.MovieContract.BASE_CONTENT_URI;
import static com.sameetahmed.themovieapp.data.MovieContract.PATH_MOVIES;
...
public class DetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPosterPath = findViewById(R.id.poster_path);
        mMovieTitle = findViewById(R.id.tv_title);
        mReleaseYear = findViewById(R.id.tv_release);
        mPlot = findViewById(R.id.tv_overview);
        ...
        if (mMovieData == null) { // If the movie data is null, then we came from the favorite movies and we need to request the details
            ...
            loadData(buildFavoriteMovieDetailsUrl(mMovieData.getId()));
            loadData(buildRequestTrailerUrl(mMovieData.getId()));
        } else {
            loadData(buildRequestTrailerUrl(mMovieData.getId())); // We already have the movie details from Main, just need to get trailers
            setDetails();
        }
    ...
    private void setDetails() {
        ...
        mVoteAverage.setText(Double.toString(mMovieData.getPopularity()));
        mMovieId = mMovieData.getId(); // Gets the MovieID to pass to the String Request to get trailers
    ...
    private boolean isMovieFavorite() {
        ...
        if (movieToQuery != null  && movieToQuery.getCount() > 0) {
            return true;
        }
        return false;
    }
